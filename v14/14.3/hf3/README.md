...
* ACI0070568 ランタイムエラーが発生した場合，エラーダイアログの詳細ボタンをクリックしたときに表示されるプロセス番号が不正確でした。トレースを実行し，[Current process](http://doc.4d.com/4Dv14/4D/14.3/Current-process.301-1697114.ja.html)から返される番号と違うプロセス番号が表示されます。

* ACI0089373 日本語版の4DPop "XLIFF" コンポーネントはロードするとエラーが返されました。

* ACI0091692 Windows版のみ，クイックレポートのカラム自動幅が適正ではありませんでした。

* ACI0090302 ストラクチャエディター上でテーブルを```delete```キーで削除した後，矢印キーを入力すると，空のエラーダイアログが表示されましたs。

* ACI0085624 ジャーナルの統合中にエラー#[1277](http://doc.4d.com/4Dv14/4D/14.3/Database-Engine-Errors-10600-4004.300-1696681.ja.html)が発生した場合，メッセージにひとつ前のオペレーション番号が表示されました。
...
* ACI0087248 MacのタッチパッドまたはMagicマウスでメソッドエディターをスクロールした場合，速度が速すぎて制御が困難でした。
...
* ACI0091466 プリント時可変のフィールドとそうではない4D Writeエリアが配置されたフォームを印刷すると，エリアが繰り返し出力されました。
