...
* ACI0098706 Windows版のみ。4D Write Proツールバーの『フォントサイズ』ポップアップメニューが正しく動作しませんでした。ボタンが『フォーカス可』に設定されているためです。

* ACI0098704 Mac版のみ。HTTP接続ハンドラーが突発的にクラッシュしました。

* ACI0098703 Windows 64ビット版のみ。クイックレポートエディターの書き出しダイアログで拡張子（例：``.txt``）を省略した場合，ファイルを書き出すことができませんでした。

* ACI0098694 データベースを再起動した後，テキスト入力エリアに設定された選択リストがフォームエディターのプロパティリストには『なし』と表示されました。フォームの実行には影響ありません。プロパティリストの問題です。
...
* ACI0098735 Windows版のみ。ODBCデータソース名（DSN）の『設定』ボタンがクリックできませんでした。ODBCドライバーの名称に『Rx』という名称が含まれていることが関係しているようです。
