...
* ACI0097581 Windows版クライアントのみ。クライアントプロセスのステータスが「延期」のときに，タスクマネージャーからクライアントを強制的に終了すると，ユーザープロセス（たとえば，メニューから起動した新規プロセス）が残されました。サーバー側からリソースが転送された場合（たとえば，ローカルリソースを強制的に更新した後など）にだけ問題が発生します。
