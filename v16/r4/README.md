...
* ACI0097360 Mac版のみ。ビルドしたサーバーアプリケーションがクラッシュしました。アプリケーションに署名をしなくても，``_CodeSignature``フォルダーがデフォルトで作成されることが原因です。
...
* ACI0097335 Mac版のみ。継承されたフォームのオブジェクトに対して``OBJECT DUPLICATE``を実行した場合，フォームを閉じたときにアプリケーションがクラッシュしました。
