...
* ACI0094057 Windows版のみ。Word文書を4D Writeで開いた場合，``BMP``, ``JPEG``, ``PNG``, ``TIFF``画像が失われました。``PICT``画像だけが保持されました。また，「サポートされない画像フォーマットです」というエラーメッセージが表示されました。
