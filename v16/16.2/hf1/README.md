...
* ACI0097321 セレクション型のリストボックスをストラクチャ間でコピーした場合，コピー先でフィールド参照が失われました。テーブス数が1個であり，リストボックスの列に設定されているのがそのテーブルであれば，フィールドの参照もコピーすることができます。テーブス数が2個以上であり，2個目のテーブルがリストボックスの列に設定されている場合に問題が発生します。
