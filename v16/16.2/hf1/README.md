...
* ACI0097321 セレクション型のリストボックスをストラクチャ間でコピーした場合，コピー先でフィールド参照が失われました。テーブル数が1個であり，リストボックスの列に設定されているのがそのテーブルであれば，フィールドの参照もコピーすることができます。しかし，テーブル数が2個以上であり，2個目のテーブルがリストボックスの列に設定されている場合にはフィールドの参照が列と一緒にコピーされません。
