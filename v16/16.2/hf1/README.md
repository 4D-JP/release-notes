...
**注記**: すべてのフィールドや変数で問題が発生するわけではありません。まず，フォームオブジェクトの「変数」を作成し，プロパティリストでこれを「フィールド」に変更した場合に問題が発生します。

* ACI0097235 64ビット版のみ。ラベルエディターではテーブルの入力フォームを指定することができますが，そのフォームのメソッド内でプロジェクトメソッドが呼ばれている場合，印刷プレビューで「プロジェクトメソッドがみつかりません」というエラーが返されました。直接，プリンターに出力すれば，問題ありません。
