...
* ACI0069623 UUIDフィールドを``SET FIELD VALUE NULL``でヌルにセットすることができませんでした。 

* ACI0095496 SQLデータをリストボックスに表示した場合，自動計算フッターは実数の代わりに整数で計算され，合計値などが正しくありませんでした。

* ACI0095540 Mac版のみ。ランタイムエクスプローラーが表示されている状態で4D Serverを終了すると，アプリケーションがクラッシュすることがありました。

* ACI0095581  Mac版のみ。リストボックスのセルに日本語を入力しているとき，変換を確定せずに別のオブジェクトをクリックすると，入力中の文字列が失われました。

* ACI0095623 ``REDUCE SELECTION([Table];0)``と``LONGINT ARRAY FROM SELECTION``にクエリが一度も実行されなかった場合，エラー``-9977``が返されました。

* ACI0093483 4D Write Proエリアおよび詳細マーカーの位置によっては，``Print form``で最初に空のページが出力されることがありました。

* ACI0094466 ``MESSAGES OFF``は``BUILD APPLICATION``に対して効果がありませんでした。

* ACI0094654 64ビット版のみ。ラベルエディターの「レイアウト」ページに移動しようとすると，エラー``59``が返されました。

* ACI0094995 メソッドエディターに日本語を入力している間，テキストが点滅しました。

* ACI0094997 64ビット版のみ。``SVG_Copy``は空のSVG参照を返しました。 

* ACI0095018 ``OPEN SETTINGS WINDOW("/Database";True;User settings) ``をユーザー設定の外部ファイル保存が無効にされている状態で実行するとアプリケーションがクラッシュしました。

* ACI0095163 フランス語版のみ。メソッドエディターに表示される``ST FIXER TEXTE BRUT``のヘルプTipsが英語版のもの（``ST SET PLAIN TEXT``）でした。 
