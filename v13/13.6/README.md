...
* ACI0088511 データベース間でフォームをコピーした場合，Resourcesフォルダーの画像が複製されないことがありました。

* ACI0088435 サブフォームが含まれる継承フォームを閉じると，アプリケーションがクラッシュしました。

* ACI0087428 **13.4以降**，システムテーブルの```INDEX_ID```仮想フィールドでJOINが実行できませんでした。

* ACI0087080 **13.4以降**，データのインポートが遅くなりました（例: 15秒だった処理が80秒に）。

* ACI0086900 255文字のフィールドに対してフォーム上で文字列を削除した場合，```On Data Change```イベントのタイミングで『このフィールドは0文字以上でなければなりません』というエラーメッセージが表示されました。

* ACI0085233 シートウィンドウに重ねて別のシートウィンドウを表示した場合，第2のシートウィンドウを閉じた後，裏に隠れていたシートウィンドウが再描画されませんでした。

* ACI0081359 セレクション型リストボックスのヘッダーをクリックした後，別のヘッダーをクリックした場合，最後にクリックしたほうのヘッダーではなく，前のヘッダーに並び替えアイコン表示されました。

* ACI0081141 Windows 7で地域と言語設定がウェールズ語（英国）に設定されている場合，4Dが起動直後にクラッシュしました。起動後の設定を変更すれば，問題ありません。

* ACI0080497 モーダルウィンドウの背面に別のモーダルウィンドウが隠れてしまった場合，スペースバーで順序を復元することができますが，最前面ウィンドウにラジオボタンやチェックボタンに反応するオブジェクト，あるいはスペースバーがショートカットに設定されたボタンが配置されている場合，それらのオブジェクトが入力を受け付けてしまい，ウィンドウの順序を戻すことができませんでした。背面のウィンドウにそのようなオブジェクトがあれば，そちらがスペースバーに反応しました。

* ACI0077434 [IMPORT DATA](http://doc.4d.com/4Dv13/4D/13.5/IMPORT-DATA.301-1458280.ja.html)で『置換』オプションを選択しても，レコードはカレントセレクションに追加されました。

* ACI0043974 [EXPORT ODBC](http://doc.4d.com/4Dv13/4D/13.5/EXPORT-ODBC.301-1458273.ja.html)は，ダイアログを省略するための```*```オプションを無視しました。

* ACI0088740 データベース間でピクチャボタンのコピー＆ペーストができませんでした。ドラッグ＆ドロップにより作成されたオブジェクトのピクチャ名/ID（Resourcesフォルダー内のパス）に余計な```#```が追加されました。

* ACI0088582 4D TAGで自動的に生成されたピクチャ変数に対するURL (例: ```http://127.0.0.1/4Dimg/4DVar.onePic/2000910177```) の最後のスラッシュ以降を削除してリクエストした場合，4Dがクラッシュしました。
