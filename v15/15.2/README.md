...
* ACI0094927 階層リストボックスを右方向いっぱいに水平スクロールさせた状態で[OBJECT SET SCROLL POSITION](http://doc.4d.com/4Dv15/4D/15.1/OBJECT-SET-SCROLL-POSITION.301-2685673.ja.html)で垂直水平スクロールを実行すると，垂直スクロール位置が正しくありませんでした。ACI0092932と同じ問題です。

* ACI0094612 クエリエディターでクエリを実行した後，もう一度クエリエディターを開くと，入力エリアにテーブル名が重複して表示されました。

* ACI0094865 オブジェクト型リストボックス（4D View Pro）の列で``On Before Data Entry``イベントが有効にされており，ドロップダウンメニュー・3ステートチェックボックス・ボタンが設定されている場合，その列が入力不可になりました。

* ACI0094776 プリント実行中，リストボックスに対して[OBJECT GET BEST SIZE](http://doc.4d.com/4Dv15/4D/15.1/OBJECT-GET-BEST-SIZE.301-2685666.ja.html)を実行すると， アプリケーションがクラッシュしました。
...
* ACI0094802 ``SMTP_SetPrefs``で設定したエンコーディングが件名に適用されませんでした。
...
* ACI0094899 ポルトガル語版の4D Viewは``Localized.rsrc``リソースファイルが欠落していました。
