...
* ACI0093707 v14以降，PV Color to indexは常に``0``を返しました。

* ACI0093820 Mac版のみ。MSCの『データ』画面と『ストラクチャ』を切り替えるたびに，メモリが減ってゆき，繰り返していると，最終的にはクラッシュしました。

* ACI0093751 Windows版のみ。デバッガーの『式』ペインにプロジェクトメソッドを入力すると，アプリケーションがクラッシュすることがありました。Alturaの問題でした。``SHOW WINDOW``など，ウインドウコマンド全般が関係しています。

* ACI0093558 Mac版のみ。ライセンス更新画面のデフォルトボタンは，Yosemite以降でも文字カラーがブラックでした。

* ACI0091602 ``VARIABLE TO VARIABLE``のコピー先変数にポインター（たとえば未定義のフォームローカル変数）を渡し，コンポーネント内で使用した場合，何も起きませんでした。ACI0058356が修正されたことの副作用です。修正により，``VARIABLE TO VARIABLE``および``SET PROCESS VARIABLE``は，コピー先変数がプロセス変数でない場合，プロセス番号を無視するようになりました。

* ACI0091224 Webログファイル（Extended Log Format, ELF形式）は，``TRANSFER_TIME``列の内容が変でした。時折，実際よりもずっとおおきな値が記録されました。

* ACI0093546 データファイルの圧縮オプションで『ログファイルを作成しない』を選択しても，ログファイルが作成されました。

* ACI0094042 Windows版のみ。Internet Explorer 11からスタイル付きテキストをペーストした場合，テキストの一部が欠落しました。``<BR>``タグが含まれる場合に問題が発生しました。Firefoxでは問題ありません。 

* ACI0093915 モーダルダイアログを閉じた後，親フォームが最善面に復帰しないことがありました。

* ACI0093826 v2004から変換したストラチャにアクセント記号が含まれるメソッド名が存在する場合，変換後に『メソッドがみつかりません』エラーが返されることがありました。非Unicodeモードにおける文字列リテラルのコード変換が原因でした。Unicodeモードであれば問題ありません。

* ACI0093883 受信したHTTPリクエストのURLにスペースが含まれる場合，``505 HTTP Version not supported``スタータスコードが返されました。``%20``であれば，問題なくリクエストが通ります。
...
* ACI0093080 Windows版のみ。カラーピッカーで選択した色が4D Viewおよび4D Writeに正しく反映されませんでした。青と赤の値が逆でした。

* ACI0093130 [SVG_SET_SHAPE_RENDERING](http://livedoc.4d.com/SVG-15/-/SVG-SET-SHAPE-RENDERING.301-2048827.ja.html)に``optimizeSpeed``を渡すとエラーが返されました。

* ACI0093309 Mac 64ビット版のみ。メニューエディターのプレビュー表示が崩れていました。QuickDrawアイコンが使用されているためです。

* ACI0093755 ストラクチャに名前がない（空の文字列）リストが存在する場合，クエリエディターが表示された後にリスト画面も表示されます。
