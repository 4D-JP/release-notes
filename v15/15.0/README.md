...
* ACI0092062 クエリエディターを開き，「~時間である」検索を実行した後，再びクエリエディターを開いて「であるかそれよりも前である」検索に切り替えると，値が```HH:MM:SS```ではなく，```HHMMSS```フォーマットで表示されました。

* ACI0092073 フォームの2個のテキスト入力エリアとフォーカス不可のボタンを配置し，そのオブジェクトメソッドでテキスト入力エリアを入力不可にするコード（```OBJECT SET ENTERABLE```）を実行した場合，テキスト入力エリアが不完全に再描画されました。

* ACI0092077 Mac版で，``` LOG EVENT```の```Into 4D Debug Message```を使用しても，コンソールにメッセージが書き込まれませんでした。

* ACI0092079 クライアントでオブジェクト変数に日付を代入した場合，データベース設定は無視され，常にローカルタイムゾーンが使用されました。スタンドアロン版であれば，```SET DATABASE PARAMETER(JSON use local time;0)```を実行により，時間帯に関係なく，日付だけを代入することができます。

* ACI0092081 特定の条件が揃った場合，フッターに計算関数が設定されたリストボックスはクラッシュしました。

1. カレントセレクション型のリストボックス
2. フィールドにインデックスが設定されている
3. フッターに「カウント」の 計算関数が設定されている
4. テーブルがREAD ONLYである
5. セレクションが空ではない

* ACI0092127 Yosemiteでフルスクリーンモードを解除すると，デザインモードのツールバーがメニューバーに半分埋もれた状態で表示されました。

* ACI0092174 Windows版のみ，システムのデフォルトプリンターが設定されていない環境で```PRINTERS LIST```を実行すると，空の配列が返されました。

**注記**: システムのデフォルトプリンターが設定されていない場合，下記のレジストリは空です。

```
HKEYCU\Software\Microsoft\WindowsNT\CurrentVersion\Windows
```

* ACI0092203 4DのHTTPおよびSOAPクライアントコマンドは，クライアント証明書を使用できるようになりましたが，デフォルトで```ClientCertificatesFolder```が証明書フォルダーに設定されていませんでした。[HTTP SET CERTIFICATES FOLDER](http://doc.4d.com/4Dv15/4D/15/HTTP-SET-CERTIFICATES-FOLDER.301-2007238.ja.html)で明示的に指定すれば問題ありません。

* ACI0092218 Mac版のみ，```PICT```形式のピクチャが配置されたフォームを```PAGE SETUP```および```Print form```でプリントしようとすると，```PAGE BREAK```でアプリケーションがクラッシュしました。Apple側のバグが原因であり，QuickDrawというよりも，PDF画像の処理に問題がありました。
...
* ACI0092390 14R5以降，Mac版のみ，プレースホルダーの文字列が日本語だけで構成されていた場合，テキストが少し浮いた位置に表示されました。1文字でも英数字が含まれていれば問題ありません。
...
* ACI0092454 詳細フォームにリストボックスなどの「ダブルクリックで画面を切り替える」オブジェクトが配置されている場合，遷移した先のダイアログを終了して元のフォームに戻ると，点滅するカーソルが表示されませんでした。

* ACI0092485 フォームのプロパティで「ユーザー更新可」を解除してからフォームを閉じ，再び開くと，プロパティは解除されていませんでした。

* ACI0092539 SQLの```INTO LISTBOX```で続けて結果をリストボックスに表示すると，2度目の検索で結果が返されなかった場合，スタンドアロン版ではリストボックスの内容がクリアされましたが，クライアント/サーバー版ではクリアされませんでした。

* ACI0092551 ```Print object```でリストボックスを印刷した場合，スタイル付きではない列の描画（改行）が正しくありませんでした。

* ACI0092593 14R5以降，Windows版のみ，メニューバーが表示されていないときに```GET WINDOW RECT```から返される値が正しくありませんでした。

* ACI0092616 リストボックスの複数行「自動」列に長さが列の幅を超えるテキストを表示した場合，水平揃えが正しく処理されませんでした。スタイル付きテキストであれば問題ありません。

* ACI0092652 ```HTTP Request```で```TRACE (HTTP trace method)```コマンドを送信した場合，レスポンスにはヘッダーだけが返され，ボディは返されませんでした。[RFC2616](http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.8)によると，リクエストが有効であれば，```Content-Type```が```message/http```であるリクエストメッセージ全体がボディに返されるべき(SHOULD)です。

* ACI0092724 ツールボックスのスタイルシートページで「ボールド体」「イタリック体」などのチェックボックスだけを操作して画面を閉じた場合，変更が保存されませんでした。

* ACI0092736 フォームエディターの「実行」ボタンで表示したフォームのフォームメソッドで```TRACE```を実行した後，デバッガをアボートした場合，それ以後はフォームイベントが実行されなくなりました。

* ACI0092741 メソッドのエディターにSQLの```EXTRACT```をタイプ入力し，続けて"("（括弧）を入力すると，アプリケーションがクラッシュしました。

```sql

ARRAY DATE($AsOfDate_ad;0)

Begin SQL
 SELECT As_Of_Date
 FROM Collection_Targets
 WHERE EXTRACT( //← ここでクラッシュ
 INTO :$AsOfDate_ad;
End SQL
```

* ACI0093003 14R4以降，ダイナミックポップアップメニューのアイテムには，```SET MENU ITEM STYLE```でスタイルを設定することができますが，スタイルに加えてアイコンも設定されていると，アイテムテキストが表示されませんでした。
