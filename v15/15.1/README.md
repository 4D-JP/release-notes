...
* ACI0093946 v12のデータファイルを変換した場合，自動UUIDタイプのプライマリーキーに存在するNULLが自動的に新しい値で更新されませんでした。本来であれば，自動タイプのプライマリーキーは，NULLが含まれていたとしても，データベース起動時に新しいキー値が自動的に発行されるはずです。整数/自動インクリメント型のプライマリーキーであれば，問題ありません。ACI0087170（自動UUIDタイプのフィールドに対してSQLのCOUNT...WHERE...IS NULLが使用できない）やACI0093762が修正されたことに関連があります。
